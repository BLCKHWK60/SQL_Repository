with irf as (
  select
    ...
    where 1=1
     and irf.START_DATE_TIME_KEY >= (select min(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 35)  
 and irf.START_DATE_TIME_KEY <= (select max(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 31)
and media_type_key = 1
  ...
  itf as (select
    ...
    where 1=1
and START_DATE_TIME_KEY >= (select min(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 35)  
 and START_DATE_TIME_KEY <= (select max(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 31)
and media_type_key = 1 
