...
(case when pcs1.survey_iq1 != '-1' then pcs1.survey_iq1 end) as q1,  
(case when pcs2.survey_iq2 != '-1' then pcs2.survey_iq2 end) as q2, 
(case when pcs2.survey_iq3 != '-1' then pcs2.survey_iq3 end) as q3, 
(case when pcs2.survey_iq4 != '-1' then pcs2.survey_iq4 end) as q4, 
(case when pcs4.survey_iq5 != '-1' then pcs4.survey_iq5 end) as q5   
...
Where 
irf.START_DATE_TIME_KEY >= (select min(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 2)  
...
where
 ...
 and irf.START_DATE_TIME_KEY <= (select max(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 1) 
  ...
  --- Voice and Chat Based Survey Report ---
...
Select  
    ...
    sa.GUID, 
    r1.Agent as Agent,  
   (case when r1.Queue NOT IN ('GSYS_VoiceHealthCheck_VQ', 'ChatHealthTestQueue') then r1.Queue end) as Queue, 
    sa.Q1, 
    sa.Q2, 
    sa.Q3, 
    sa.Q4, 
