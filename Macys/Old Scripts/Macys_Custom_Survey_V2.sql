...
and   
irf.START_DATE_TIME_KEY >= (select min(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 18)  
and 
...
from Survey_Answers sa
    ...
    join    (select    
            ...
            join 
                (SELECT RESOURCE_GI2.* FROM RESOURCE_GI2 WHERE RESOURCE_TYPE_CODE='AGENT' AND RESOURCE_SUBTYPE='Agent') a12
