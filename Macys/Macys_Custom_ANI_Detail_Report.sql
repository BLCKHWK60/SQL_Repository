 ...
 group by
  ...
  select
    ...
    from interaction_resource_fact irf
      ...
      inner join RESOURCE_              r     on (r.resource_key = irf.last_vqueue_resource_key)
    where 1=1
     and irf.START_DATE_TIME_KEY >= (select min(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 3)  
...
and media_type_key = 1
  ...
  itf as (select
    ...
    from interaction_fact
    where 1=1
and START_DATE_TIME_KEY >= (select min(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 3)  
 and START_DATE_TIME_KEY <= (select max(date_time_key) as Label_key from date_time where date(cal_date) = current_date - 1)
and media_type_key = 1 
